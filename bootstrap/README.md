...
- `SET_YOUR_PROJECT_NAME` -> your project name (e.g. `mycompany`)
- `SET_YOUR_MANAGEMENT_ACCOUNT_ID` -> your management AWS account ID
- `SET_YOUR_DOMAIN.com` -> your domain (e.g. `mycompany.com`)
- `SET_YOUR_GITHUB_ORG` -> your GitHub organization (e.g. `mycompany`)
- `YOUR_EMAIL@example.com` -> your email for SSO user

```bash
# Find all placeholders
grep -r "SET_YOUR" --include="*.tf" --include="*.md" .
```

## Step 5: Apply Organizations

This creates the AWS Organization, OUs, member accounts, and SCPs.
...
First, update `aws/organizations/management/identity/main.tf` to add your team's SSO users.
...
Creates the `terraform` execution role and GitHub OIDC provider in each account.
...
2. Set up CI/CD using the GitHub OIDC roles (see README)
...
**Permission denied during assumes role**
